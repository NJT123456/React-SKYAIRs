...
const AppProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const cities_start_date = new Set(
      searchResults.map((val) =>
        dayjs(val.depart_date).format("ddd, DD MMM YYYY") 
        // ===
  ...
  return (
    <AppContext.Provider
      value={{
        ...
        setValueGo,
        unique,
        setUnique,formatNumber,selectFlight, setSelectFlight,isReturn, setIsReturn
      }}>
      {children}
    </AppContext.Provider>
  )
